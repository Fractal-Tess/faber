...
use crate::config::{DevicePermissions, FilePermissions, FolderPermissions, MountsConfig};
...
pub fn setup_mounts(root: &Path, mounts: &MountsConfig) -> Result<(), ContainerError> {
    debug!("Setting up mounts for {}", root.display());

    mount_folders(root, mounts)?;
    mount_tmpfs(root, mounts)?;
    mount_devices(root, mounts)?;
    mount_files(root, mounts)?;
...
pub fn umount_all(root: &Path, mounts: &MountsConfig) -> Result<(), ContainerError> {
    ...
    if let Err(e) = umount_files(root, mounts) {
        warn!("Unmount files failed at {}: {}", root.display(), e);
        last_err = Some(e);
    }
    if let Err(e) = umount_devices(root, mounts) {
        warn!("Unmount devices failed at {}: {}", root.display(), e);
        last_err = Some(e);
    }
    if let Err(e) = umount_tmpfs(root, mounts) {
        warn!("Unmount tmpfs failed at {}: {}", root.display(), e);
        last_err = Some(e);
    }
    if let Err(e) = umount_folders(root, mounts) {
...
/// Mounts host directories into the container with security restrictions.
///
/// This function performs bind mounts of host directories into the container
/// filesystem. It follows a three-step security process for each mount:
///
/// 1. **Initial bind mount** - Recursive bind to capture any submounts
/// 2. **Private propagation** - Isolate mount events from the host
/// 3. **Security remount** - Apply nosuid, nodev, and optional readonly flags
///
/// The recursive bind mount ensures that any submounts within the source
/// directory are also available in the container, which is important for
...
///
/// # Returns
/// * `Ok(())` - All folder mounts completed successfully
/// * `Err(ContainerError)` - Any mount operation failed
///
/// # Security Features
/// - **MS_NOSUID**: Prevents setuid/setgid programs from gaining privileges
/// - **MS_NODEV**: Prevents device file access (except for device mounts)
/// - **MS_RDONLY**: Applied for read-only folder permissions
/// - **MS_PRIVATE**: Prevents mount propagation to/from host namespace
fn mount_folders(root: &Path, mounts: &MountsConfig) -> Result<(), ContainerError> {
    for m in &mounts.folders {
        ...
        // Initial bind (recursive) to bring submounts, then set propagation private, then remount with security flags
        let initial_flags = MsFlags::MS_BIND | MsFlags::MS_REC;
        mount(
        ...
        // Set mount propagation to private (recursive)
        mount(
            Option::<&str>::None,
            &target,
            Option::<&str>::None,
...
fn mount_tmpfs(root: &Path, mounts: &MountsConfig) -> Result<(), ContainerError> {
...
fn mount_devices(root: &Path, mounts: &MountsConfig) -> Result<(), ContainerError> {
    for m in &mounts.devices {
        ...
        // Initial bind (no nodev here to preserve device semantics). Then set private propagation. Then optional ro remount.
        let initial_flags = MsFlags::MS_BIND | MsFlags::MS_REC | MsFlags::MS_NOSUID; // keep device semantics
        mount(
            Some(Path::new(&m.source)),
            &target,
            Option::<&str>::None,
...
fn mount_files(root: &Path, mounts: &MountsConfig) -> Result<(), ContainerError> {
...
pub fn mount_essential_filesystems(root: &Path) -> Result<(), ContainerError> {
    // /proc
